 ...
 * @summary 투표 삭제
 */
export const deletePoll = (pollId: number, options?: SecondParameter<typeof customInstance>) => {
  return customInstance<string>({ url: `/v1/polls/${pollId}`, method: 'DELETE' }, options);
};
...
export const findAllPolls = (
  ...
  return customInstance<Poll[]>({ url: `/v1/polls`, method: 'GET', signal }, options);
};
...
>(options?: {
  ...
  const queryFn: QueryFunction<Awaited<ReturnType<typeof findAllPolls>>> = ({ signal }) =>
    findAllPolls(requestOptions, signal);
 ...
 * @summary 새로운 투표 생성
 */
export const createPoll = (poll: Poll, options?: SecondParameter<typeof customInstance>) => {
  return customInstance<Poll>(
    {
      url: `/v1/polls`,
      method: 'POST',
